...
(defn read-file
  ...
  ([file-name pc]
   (-> (fr/read-file file-name pc)
       ...
       (assoc-in [global-key process-context-key]
                 (imp/select-module-node-processor pc)))))



(defn has-dml-type? [m-map]
  (let [dml (dml-key m-map)]
    (or
      (= dml-update-key dml)
      (= dml-call-key dml)
      (= dml-insert-key dml)
...
(defn node-processor
  [tms]
  (if-let [w (get-in tms [global-key process-context-key])]
    w
...
(defn pull-default-request!
  ...
  (let [dfmat (if (or gname
                      (sequential? name))
                {:pformat map-format :rformat nested-join-format}
                {:pformat map-format :rformat map-format})
        request-m (merge dfmat request-m)
        request-m (if gname
                    (assoc request-m :rformat nested-join-format)
                    request-m)]
    request-m))
...
(defn pull-sequence-format!
  [request-m]
  (merge request-m {:pformat map-format :rformat value-format}))
...
(defn push-default-request!
  ...
  (let [d (if (or gname
                  (sequential? name))
            {:pformat nested-map-format :rformat nested-map-format}
            {:pformat map-format :rformat map-format})
        request-m (merge d request-m)
        request-m (if gname
                    (-> request-m
                        (assoc :pformat nested-map-format)
                        (assoc :rformat nested-map-format))
                    request-m)]
    request-m))
...
(defn pull
  ...
  [ds tms & {:as request-m}]
  (if-let [w (cc/failed? (validate-input! request-m))]
    w
...
(defn- execution-log
  [tm-coll]
  (let [v (mapv #(select-keys % [sql-key exec-time-total-key exec-time-start-key]) tm-coll)
        w (mapv (fn [t]
                  (update-in t [exec-time-start-key] (fn [o] (str (as-date o))))
                  ) v)]
    (log/info w)))
