...
(deftest do-join-test
  (testing "test do-join "
    (let [join [[:tab :id :dadysql.spec/one-many :tab2 :tab-id]
                [:tab :id :dadysql.spec/one-many :tab3 :tab-id]]
  ...
  (testing "test do-join "
    (let [join [[:tab :id :dadysql.spec/one-one :tab1 :tab-id]
                [:tab :tab4-id :dadysql.spec/many-one :tab4 :id]
                [:tab :id :dadysql.spec/many-many :tab2 :id [:tab-tab1 :tab-id :tab2-id]]]
...
(deftest group-by-target-entity-one-test
  (testing "test dest-rel-data "
    (let [data {:tab {:id   100
          ...
          j [[:tab] :id :dadysql.spec/one-many :tab1 :tab-id]
  ...
  (testing "test dest-rel-data with single join "
    (let [data {:tab [{:id   100
                      ...
                      {:id   102
                       :tab1 {:tab-id 138}}]}
          j [[:tab 0] :id :dadysql.spec/many-many [:tab 0 :tab1 0] :tab-id [:ntab :tab-id :tab1-id]]
...
(deftest do-disjoin-test
  (testing "test do-disjoin with :1-n relationship "
    (let [join [[:tab :id :dadysql.spec/one-many :tab1 :tab-id]]
          data {:tab {:id   100
  ...
  (testing "test do-disjoin with :n-n relationship "
    (let [join [[:tab :id :dadysql.spec/many-many :tab1 :tab-id [:ntab :tab-id :tab1-id]]]
          data {:tab [{:id   100
                       :tab1 [{:tab-id 100}
                              {:tab-id 101}]}
                      {:id   102
      ...
      (is (= actual-result
             expected-result))))
  (testing "test do-join"
    (let [j [[:employee :id :dadysql.spec/one-one :employee-detail :employee_id]]
          data {:employee {:firstname       "Schwan",
                           :lastname        "Ragg",
                           :dept_id         1,
                           :transaction_id  0,
...
(deftest group-by-join-src-test
  (testing "test group-by-join-src "
    (let [v [[:tab :id :dadysql.spec/one-many :tab1 :tab-id]]
          expected-result {:tab {:dadysql.spec/join [[:tab :id :dadysql.spec/one-many :tab1 :tab-id]]}}
...
(deftest join-emission-batch-test
  (testing "test join-emission-batch "
    (let [join [[:tab :ID :dadysql.spec/one-many :tab2 :tab-id]
                [:tab :id :dadysql.spec/one-many :tab3 :tab-ID]
                [:tab :id :dadysql.spec/many-many :tab1 :tab-id [:ntab :tab-ID :tab1-id]]]
          expected-result [[:tab :id :dadysql.spec/one-many :tab2 :tab-id]
                           [:tab :id :dadysql.spec/one-many :tab3 :tab-id]
                           [:tab :id :dadysql.spec/many-many :tab1 :tab-id [:ntab :tab-id :tab1-id]]]
