...
#_(clojure.pprint/pprint
  (get-in (read-file "tie.edn.sql") [:get-dept-employee :spec-model.core/join]))
     ...
     ; (clojure.pprint/pprint (:get-meeting-by-id w))
      ...
      (do
       ...
       ;   [[:id :type (resolve 'int?) "Id will be Long "]] (get-in w [:get-dept-by-id validation-key])
          [[:department :id :spec-model.core/rel-1-n :employee :dept_id]] (get-in w [:get-dept-by-id :spec-model.core/join])
        ...
        (are [expected actual]
          ...
          [[:employee :id :spec-model.core/rel-1-1 :employee-detail :employee_id]
           ...
           [:employee :dept_id :spec-model.core/rel-n-1 :department :id]] (get-in w [:get-dept-employee :spec-model.core/join]))
        ...
        (are [expected actual]
          ...
          [[:meeting
            ...
            :employee
            :id
            [:employee-meeting :meeting_id :employee_id]]] (get-in w [:get-meeting-by-id :spec-model.core/join])))))
  )
