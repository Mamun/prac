...
(defn do-grouping2 [m]
  (let [{:keys [reserve others]} (-> (get-in m [:global reserve-name-key])
                                     (group-by-reserve-key (:module m)))]
    (hash-map :global (:global m) :reserve reserve :module others)))
...
(defn compile-one-config2 [config]
  (->> (update-in config [:name] second )
...
(defn do-compile2 [coll]

  (if-not (s/valid? :dadysql.compiler.spec/spec coll)
    (throw (ex-data (s/explain :dadysql.compiler.spec/spec coll)  ))
    (let [w (s/conform :dadysql.compiler.spec/spec coll)

          {:keys [global module reserve] :as w} (do-grouping2 w)

          _ (clojure.pprint/pprint w)
          f-config (compile-one-config2 global)
          batch-steps (comp
                        (map #(compile-one f-config %))
                        cat)
          batch-result (->> (into [global] batch-steps module)
                            (concat reserve))]
      (distinct-name! batch-result)
      (->> batch-result
           (dcsu/compiler-emit)
...
(s/fdef hello
    :args (s/cat :a :dadysql.compiler.spec/spec)
    :ret ::s/any    )
